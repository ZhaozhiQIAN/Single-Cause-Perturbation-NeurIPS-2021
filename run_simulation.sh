...
#        for method in dor scp propensity tarnet vsr
        ...
        do
            python -u run_simulation.py --method=${method} --config=${config} > results/${method}_${config}.txt
        done

#        python -u run_simulation_scp.py --method=ablation  --config=${config} > results/scp_ablation_${config}.txt
    done
done
