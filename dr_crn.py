...
class TarNet(nn.Module):
    def __init__(self,  n_confounder, n_cause, n_outcome, n_hidden, mmd_sigma=0., lam_mmd=0., device=DEVICE):
        super().__init__()
...
class DR_CRN(nn.Module):
    def __init__(self, single_cause_index, n_confounder, n_cause, n_outcome,
        ...
        if linear:
            self.confounder_rep_net = nn.Sequential(
                nn.Linear(n_input, n_confounder_rep),
            ).to(device)
    
            self.outcome_rep_net = nn.Sequential(
                nn.Linear(n_input, n_outcome_rep),
            ).to(device)
        else:
            self.confounder_rep_net = nn.Sequential(
                nn.Linear(n_input, n_input + 1),
                nn.ReLU(),
                nn.Linear(n_input + 1, n_confounder_rep),
            ...
            self.outcome_rep_net = nn.Sequential(
                nn.Linear(n_input, n_input + 1),
                nn.ReLU(),
                nn.Linear(n_input + 1, n_outcome_rep),
            ).to(device)
        ...
        if not self.binary_outcome:
            if linear:
                self.outcome_net0 = nn.Sequential(
                    nn.Linear(n_confounder_rep + n_outcome_rep, n_outcome),
                ).to(device)
    ...
    def forward(self, x):
        ...
        if self.single_cause_index == 0:
            ...
            all_confounders = torch.cat([x[:, :self.single_cause_index], x[:, (self.single_cause_index + 1):]], dim=-1)
...
class DR_CRN_Multicause(nn.Module):
    def __init__(self,  n_confounder, n_cause, n_outcome, n_confounder_rep, n_outcome_rep, mmd_sigma=0., lam_mmd=0., device=DEVICE):
        super().__init__()
